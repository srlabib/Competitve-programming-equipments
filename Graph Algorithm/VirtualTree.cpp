...
int32_t main() {
  ...
  dfs(1, 0); // must call for ascestor precal
  cin >> q;
  while (q--) {
    cin >> k;
    vector<int> v;
    ...
    ans = 0;
    vector<int> nodes;
    if (fl) nodes = buildtree(v); //return the nodes in the tree and form the graph in t[]
    if (fl) yo(nodes.front()); // node.front() = root
    if (!fl) ans = -1;
    cout << ans << '\n';
    // clear the tree
    for (auto x : nodes) t[x].clear();   // must clear the nodes
